...
void shahm58_2(){
    int row=4,col=4;
    int upper[4][4] = {{6,2,8,9},{0,1,3,4},{0,0,4,0}};
    int*input_upper[4];
    for (int i=0;i<4;i++){
...
void shahm58_3(){
    ...
    int set_row = 1, set_col = 3;
      int row=4,col=5;
      int data[4][5] = {{4,3,2,6,1},{6,8,1,7,5},{4,3,2,19,1},{5,12,6,7,1}};
      int*input_data[4];
      for (int i=0;i<4;i++){
          input_data[i]=data[i];
      }
...
void shahm58_4(){
    int row=4,col=4;
    int data[4][4] = {{12,6,7,9},{3,2,13,1},{8,5,3,2},{5,1,7,9}};
    int*input_data[4];
    for (int i=0;i<4;i++){
            input_data[i]=data[i];
    }
    Matrix main =  Matrix(row,col,input_data);
    Matrix copied = main.copy();
    string row1="12 6 7 9 \n", row2="3 2 13 1 \n", row3="8 5 3 2 \n",row4="5 1 7 9 \n";
    string expected=row1+row2+row3+row4;
    ASSERT_EQUAL(expected,copied.toString());
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(shahm58_1));
	s.push_back(CUTE(shahm58_2));
	s.push_back(CUTE(shahm58_3));
	s.push_back(CUTE(shahm58_4));
